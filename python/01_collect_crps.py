...
def query_puf(challenge: int) -> int:
    ...
    if gpio_out is None or gpio_in is None:
        raise RuntimeError("PYNQ GPIO is not initialized")
...
def main() -> None:
    # Import the hardware-only dependency lazily so desktop ML tooling can
    # compile and inspect this module without a PYNQ installation.
    try:
        from pynq import Overlay
    except ImportError as exc:
        raise SystemExit(
            "PYNQ is required on the FPGA board. Install it in the board's "
            "PYNQ environment before collecting CRPs."
        ) from exc

    global gpio_out, gpio_in
